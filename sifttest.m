...
function pic_sift_vector= get_pic_sift_vector(sift_image)
    m=rgb2gray(sift_image) ;
    %????????????????128????????????
    sift_pic_11 =  mysiftvector(m);
    %??????????????????????????????????
    pic_4=sort_picture(m,2);
    for y=1:4
        eval([' sift_pic_4',num2str(y),'=',' mysiftvector(pic_9{y})',';']);
    end
    %??????????????????????????????????
	...
	pic_sift_vector_func=[sift_pic_11, sift_pic_41,...
    ...
    pic_sift_vector = pic_sift_vector_func;
end
